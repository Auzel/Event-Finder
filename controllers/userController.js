...
const getUser = async function(req, res) {
    try {
        ...
        if (!user) {
            ...
            return res.status(200).json(message.response("OK", {_id: user._id, name: user.name, username: user.username, email: user.email, eventPrefs: user.eventPrefs, reviews: user.reviews}));
        }
    } catch (err) {
